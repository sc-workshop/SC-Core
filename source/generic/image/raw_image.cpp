...
namespace sc
{
	...
	void RawImage::copy(RawImage& image) const
	{
		...
		if (image.width() != m_width || image.height() != m_height)
		{
			...
			{
				base_depth_data = memalloc(Image::calculate_image_length(m_width, m_height, base_depth));
				Image::remap(m_data, base_depth_data, m_width, m_height, m_depth, base_depth);
			}

			data = memalloc(Image::calculate_image_length(image.width(), image.height(), m_depth));
			Image::resize(
		...
		{
			memcopy(data ? data : m_data, image.data(), data_size);
		}
