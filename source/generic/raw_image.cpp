...
namespace sc
{
    RawImage::RawImage(uint8_t* data, uint16_t width, uint16_t height, Image::BasePixelType type, Image::ColorSpace space = Image::ColorSpace::Linear) : m_data(data), m_type(type), m_space(space)
    {
        m_width = width;
        m_height = height;
    };
    ...
    BasePixelType RawImage::base_type() const
    {
        return m_type;
    };

    ColorSpace RawImage::colorspace() const
    {
        return m_space;
    }
